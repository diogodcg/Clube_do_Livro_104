...
<style>
  ...
  [data-testid="stSidebar"] { background-color: #0d2d5e !important; }
  ...
  [data-testid="stSidebar"] .stRadio div[role="radiogroup"] { gap: 0.5rem !important; }
  [data-testid="stSidebar"] hr { border-color: #1a4a8a !important; }

  .stApp { background-color: #f5f7fa; }
  .block-container { padding-top: 1.5rem !important; max-width: 820px !important; }

  h1, h2, h3 { font-family: 'Lora', serif !important; color: #0d2d5e !important; }
  ...
  .stLinkButton a:hover { background: #f47c20 !important; }
  ...
  .stButton > button:hover { background: #f47c20 !important; color: #ffffff !important; }

  .stTextInput > div > div > input {
    font-family: 'Lora', serif !important; font-size: 1rem !important;
    border: 1.5px solid #cdd8f0 !important; border-radius: 8px !important;
  ...
  .book-card {
    ...
    padding: 1rem 1.2rem; margin-bottom: 0.75rem; transition: box-shadow 0.2s;
  }
  .book-card:hover { box-shadow: 0 2px 12px rgba(13,45,94,0.10); }
  .book-title {
    font-family: 'Lora', serif; font-size: 1.05rem; font-weight: 600;
    color: #0d2d5e; margin-bottom: 0.25rem;
  }
  .book-meta {
    font-size: 0.8rem; color: #5a7ab0;
    display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center;
  }
  .badge {
    display: inline-block; background: #e8f0fe; color: #0d2d5e;
    border-radius: 4px; padding: 1px 7px;
    font-size: 0.7rem; font-weight: 600; letter-spacing: 0.04em;
  }

  .empty-state { text-align: center; padding: 3rem 1rem; color: #8aa3cc; }
  .empty-state .icon { font-size: 2.5rem; margin-bottom: 0.5rem; }
  .empty-state p { font-size: 0.9rem; margin: 0; color: #8aa3cc; }

  .cover-placeholder {
    width: 90px; height: 128px; background: #e8f0fe;
    border: 1px solid #cdd8f0; border-radius: 6px;
  ...
  .cl-footer {
    ...
    background: #0d2d5e; color: #a0bce8;
    text-align: center; font-size: 0.72rem;
    padding: 0.5rem 1rem; letter-spacing: 0.03em; z-index: 999;
  }
</style>
...
def tela_login():
    st.markdown("""
    <div style='
      max-width: 380px; margin: 5rem auto 0;
      background: #ffffff; border: 1px solid #dce6f7;
      border-top: 4px solid #f47c20; border-radius: 12px;
...
with st.sidebar:
    ...
    opcoes = ["🏠  Início", "🔍  Busca de Livro", "📋  Acervo Completo", "📤  Upload de Ebooks"]
    pagina = st.radio("Navegação", options=opcoes, label_visibility="collapsed")

    # Fecha a sidebar ao trocar de página (comportamento mobile)
